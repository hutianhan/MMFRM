...
if __name__ == "__main__":
    ...
    # network = Res18Feature(pretrained=imagenet_pretrained, drop_rate=args.drop_rate)
    # network =RestNet18()
    #network=EfficientNetB0(in_channels=3, n_classes=7)
    #network = MobileNetV1()
    # network=DenseNet121()
    # network = Backbone(50, 0.2)
    # network = My_VGG(num_classes=7)
    network = RestNet18().to(device)
    ...
    for t in range(1, args.epochs + 1):
        ...
        for batch_i, (imgs, targets, indexes) in enumerate(train_loader):
            ...
            #     sq=0
            #     for j  in range(6):
            #         sq=sq+math.sqrt(pow(sm[i][j]-aver,2))
            #     if targets[i]==6:
            #         a1=a1+sq
            #     else:
            #         a2=a2+sq
            ...
            # print(c0,s0,sq0,RR_loss2)
            # print("++++++++++++++++++++++++++++++++++++")


            targets = targets.to(device)
            loss = 0.9*criterion(outputs, targets) + 0.1*RR_loss2
        ...
        with torch.no_grad():
            ...
            for batch_i, (imgs, targets, _) in enumerate(val_loader):
                ...
                loss = 0.99*criterion(outputs, targets) + 0.01*RR_loss2
