...
class MobileFaceNet(Module):
    ...
    def forward(self, x):
        ...
        out = self.conv_3(out) ##### [B, 64, 28,28]
        feature1=out
        out = self.conv_34(out)
        out = self.conv_4(out)##### [B, 128, 14,14]
        feature2 = out
        out = self.conv_45(out)
        out = self.conv_5(out)##### [B, 128, 7,7]

        conv_features = self.conv_6_sep(out)    ##### [B, 512, 7, 7]
        feature3 = conv_features
        out = self.output_layer(conv_features)  ##### [B, 136]
        out2=self.fc(out)
        # return out, conv_features,out2
        # return out2

        attention_weights = self.alpha(out)
        out2 = attention_weights * self.fc(out)
        return feature1,feature2,feature3,attention_weights, out2
